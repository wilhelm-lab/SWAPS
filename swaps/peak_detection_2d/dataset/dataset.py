...
class MultiHDF5Dataset(torch.utils.data.Dataset):  # type: ignore[attr-defined]
    ...
    def _load_data_point(self, file_idx, data_point_name):
        ...
        with h5py.File(filename, "r") as f:
            ...
            if isinstance(loaded_data_point, h5py.Dataset):
                # single dataset
                if loaded_data_point.shape == ():  # scalar
                    returned_data_point[data_point_name] = loaded_data_point[()]
                else:
                    returned_data_point[data_point_name] = loaded_data_point[:]

            elif isinstance(loaded_data_point, h5py.Group):
                ...
                raise TypeError(
                    f"Expected h5py.Dataset or h5py.Group, got {type(loaded_data_point)}"
                )
    ...
    def __getitem__(self, idx):
        ...
        if self.transforms is not None:
            ...
            Logger.warning("No transforms applied to the dataset. Return target labels")
            return img.to(device), hint.to(device), target["labels"].to(device)
...
class PeptActPeakSelection_Infer_Dataset(torch.utils.data.Dataset):  # type: ignore[attr-defined]
    ...
    def __getitem__(self, idx):
        ...
        datapoint_dict = prepare_2d_act_and_mask_updated(
            pept_mz_rank=int(self.data_index[idx]),
            peptbatch_act=self.peptbatch_act,  # type: ignore
            maxquant_dict=self.maxquant_dict,
            hint_matrix=self.hint_matrix,  # type: ignore
            add_label_mask=self.add_label_mask,
            remove_blank_image=False,
        )
        # Logger.debug("datapoint_dict %s", datapoint_dict)
        assert (
            datapoint_dict is not None
        ), f"Data point with pept_mz_rank {self.data_index[idx]} is None"
        # Wrap sample and targets into torchvision tv_tensors:
...
class MultiHDF5_MaskDataset(torch.utils.data.Dataset):  # type: ignore[attr-defined]
    ...
    def _load_data_point(self, file_idx, group_name):
        ...
        with h5py.File(filename, "r") as f:
            obj = f[group_name]  # could be Dataset or Group

            if isinstance(obj, h5py.Dataset):
                # single dataset
                if obj.shape == ():  # scalar
                    data_point[group_name] = obj[()]
                else:
                    data_point[group_name] = obj[:]

            elif isinstance(obj, h5py.Group):
                # iterate over all datasets in the group
                for dataset_name, dataset_obj in obj.items():
                    if (
                        isinstance(dataset_obj, h5py.Dataset)
                        and dataset_obj.shape == ()
                    ):
                        data_point[dataset_name] = dataset_obj[()]
                    else:
                        data_point[dataset_name] = dataset_obj[:]

            else:
                raise TypeError(f"Expected h5py.Dataset or h5py.Group, got {type(obj)}")
...
class Peak2dDataset(torch.utils.data.Dataset):  # type: ignore[attr-defined]

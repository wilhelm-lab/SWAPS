...
__C.FDR.METHOD = "percolator"  # "percolator" (default; trains on all target candidates via the percolator CLI) or "mokapot_trusted" (trains only on MS/MS-confirmed targets + a balanced decoy sample; see postprocessing.rescore.brew_trusted_target_model / swaps/rescore_msms_trusted_ims.py). Ignored when FDR.ENABLED is False.
__C.FDR.MOKAPOT_TRUSTED = ConfigurationNode()
__C.FDR.MOKAPOT_TRUSTED.MODEL_TYPE = "percolator"  # "percolator" (mokapot's stock semi-supervised PercolatorModel, restricted to the trusted pool) or "supervised" (fixed-label fit, no iterative label refinement). Only used when FDR.METHOD == "mokapot_trusted"

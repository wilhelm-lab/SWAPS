...
__C.MATCH_FEATURES_KWARGS.floor_extraction.enabled = False  # if True, a two-step post-FDR correction (postprocessing.extract_noise_floor): (1) estimate_dataset_noise_level samples n_candidates_sampled random mz_ranks across all runs once per dataset (shared across every FDR.METHOD variant), pools every non-zero pixel of every sampled (mz_rank, run) image (no noise/signal segmentation), fits a 2-component Gaussian mixture to the pooled values seeded at the histogram's two local maxima (see _fit_two_component_gmm's docstring -- sklearn's default single k-means-seeded EM run was verified to reliably land in a worse, overlapping local optimum on this skewed data), and takes the low-intensity component's responsibility-weighted raw-space mean (not the naive back-transform of the fitted log-space mean, which is biased for small x); (2) apply_floor_extraction then subtracts noise_mean * area from intensity_sum IN PLACE on pp_reference, pp_match_target_filtered, and pp_match_target_msms individually (each before concatenation into pp_all), so every downstream consumer of any of those three -- build_pivot/DirectLFQ, the raw ions_raw/rank-intensity/quant-by-run plots, and calc_quant_corr's own QC plot -- sees the corrected value consistently. Runs entirely after FDR control (inside _finalize_fdr_results), so it never touches decoys or percolator training/scoring. The pre-correction value is preserved as intensity_sum_without_floor_correction; floor_background_estimate (per-row, scales with that row's own area) is also added. The GMM fit is saved as RESULT_PATH/floor_extraction_gmm_fit.png(+.svg) for inspection. See dev_swaps2.0_110_background_extraction_mix_of_gaussian.ipynb for the exploration behind this method.
__C.MATCH_FEATURES_KWARGS.floor_extraction.n_candidates_sampled = 100  # number of random mz_ranks sampled from dict_ref (no organism/species filtering -- dataset-agnostic) for noise-level estimation; each contributes one image per run it has activation data in.

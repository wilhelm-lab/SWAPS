...
__C.MATCH_FEATURES_KWARGS.floor_extraction = ConfigurationNode()
__C.MATCH_FEATURES_KWARGS.floor_extraction.enabled = False  # if True, a two-step post-FDR correction (postprocessing.extract_noise_floor): (1) estimate_dataset_noise_median samples n_candidates_sampled random mz_ranks across all runs once per dataset (shared across every FDR.METHOD variant) and takes the pooled median of extract_noise_pixels_no_smooth's output; (2) apply_floor_extraction then subtracts noise_median * area * noise_fraction from intensity_sum IN PLACE on pp_reference, pp_match_target_filtered, and pp_match_target_msms individually (each before concatenation into pp_all), so every downstream consumer of any of those three -- build_pivot/DirectLFQ, the raw ions_raw/rank-intensity/quant-by-run plots, and calc_quant_corr's own QC plot -- sees the corrected value consistently. Runs entirely after FDR control (inside _finalize_fdr_results), so it never touches decoys or percolator training/scoring. The pre-correction value is preserved as intensity_sum_without_floor_correction; floor_background_estimate (per-row, scales with that row's own area) is also added. Parameter-swept (2026-09, HYE nanoflow_5min + microflow_30min, noise_fraction 0.0-1.0): no single noise_fraction simultaneously optimal for both ECOLI and YEAST target ratios, IQR widens monotonically -- evaluate before enabling for a real run.
__C.MATCH_FEATURES_KWARGS.floor_extraction.n_candidates_sampled = 100  # number of random mz_ranks sampled from dict_ref (no organism/species filtering -- dataset-agnostic) for noise_median estimation; each contributes one raw activation image per run.
__C.MATCH_FEATURES_KWARGS.floor_extraction.seed = 789  # RNG seed for the candidate sample, for reproducibility across reruns of the same dataset.
__C.MATCH_FEATURES_KWARGS.floor_extraction.threshold = 0.0  # passed to extract_noise_pixels_no_smooth: pixels > this value are candidate signal; remove_small_objects(min_size) then decides which of those survive as real peak vs. noise. No gaussian smoothing -- validated for reproducibility across images independent of peak intensity/image size (dev_swaps2.0_110_background_extraction.ipynb).
__C.MATCH_FEATURES_KWARGS.floor_extraction.min_size = 3  # skimage.remove_small_objects min_size (8-connectivity) for the noise_median estimation step.
__C.MATCH_FEATURES_KWARGS.floor_extraction.noise_fraction = 0.2  # discount on noise_median * area since background is assumed to occupy only a fraction of a peak's own pixels, not the whole region.

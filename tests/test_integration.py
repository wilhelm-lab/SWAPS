...
# ---------------------------------------------------------------------------
# SAGE placeholder tests (skipped until data is available)
# ---------------------------------------------------------------------------
...
class TestSageParsingIntegration:
    ...
    def test_sage_parser_runs_on_real_data(self, sage_dir):
        from prepare_dict.search_engine_output_parser import sage_parser

        tsv_files = [f for f in os.listdir(sage_dir) if f.endswith(".tsv")]
        df = pd.read_csv(os.path.join(sage_dir, tsv_files[0]), sep="\t")
        result = sage_parser(df)
        assert isinstance(result, pd.DataFrame)
        assert len(result) > 0


# ---------------------------------------------------------------------------
# MaxQuant placeholder tests (skipped until data is available)
# ---------------------------------------------------------------------------
...
class TestMaxQuantParsingIntegration:
    ...
    def test_evidence_txt_loadable(self, maxquant_dir):
        evidence_path = os.path.join(maxquant_dir, "evidence.txt")

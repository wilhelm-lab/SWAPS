...
**Alternative — MS/MS-trusted-target mokapot rescoring** (functions in `rescore.py`: `select_trusted_training_rows`, `brew_trusted_target_model`): trains on only the MS/MS-confirmed (Reference/Quant_Only) targets instead of all target candidates, via mokapot (semi-supervised `PercolatorModel` or a fully-supervised fixed-label `LinearSVC`), scoring the full candidate population afterward. Wired into production via `cfg.FDR.METHOD` — a list, e.g. `["percolator", "mokapot_trusted"]` — where each entry reruns FDR control + DirectLFQ + result analysis once, into its own `quant_dir` subdir (`FDR.MOKAPOT_TRUSTED.MODEL_TYPE` selects `"percolator"` vs `"supervised"` for the `mokapot_trusted` entry). A standalone CLI also exists (`swaps/rescore_msms_trusted_ims.py`), resuming from an existing quant_dir (`--from-fdr`) same as `sbs_runner_ims.py`, useful for fitting the trusted model against a quant_dir that was already built with `FDR.METHOD: [percolator]` only.
